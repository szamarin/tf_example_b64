...
def main(args):
    ...
    # create TensorBoard callback that we can later use to analyze the model 
    ...
    # output metrics for HPO
    val_loss, val_accuracy = model.evaluate(test_ds)
    print(f"validation_loss: {val_loss}")
    print(f"validation_accuracy: {val_accuracy}")
    
    # save model
    save_dir = os.path.join(os.environ.get("SM_MODEL_DIR"), "001")
    model.save(save_dir)
    # save class names for reuse during inference
